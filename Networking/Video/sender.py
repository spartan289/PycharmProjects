...
Requirements
    ...
    - WebCamera needed
"""
import socket, pickle # Import Modules
from cv2 import cv2
...
s.setsockopt(socket.SOL_SOCKET, socket.SO_SNDBUF,
             10000000)  # setSockoptTo open two protocols,SOL_SOCKET: Request applies to socket layer.
serverip = "172.27.112.208" # Server public IP
serverport =6881  # Server Port Number to identify the process that needs to recieve or send packets
print(s)
...
while True:
    ...
    rete, buffer = cv2.imencode(".jpg", photo, [int(cv2.IMWRITE_JPEG_QUALITY),
                                               30])  # ret will returns whether connected or not, Encode image from image to Buffer code(like [123,123,432....])
    x_as_bytes = pickle.dumps(
        buffer)  # Convert normal buffer Code(like [123,123,432....]) to Byte code(like b"\x00lOCI\xf6\xd4...")
